...
async function ensureProjectReady(reuseProject = false) {
  if (getComposerContext()) return;
  if (reuseProject) { await waitForComposer(15000); return; }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'FLOW_OPEN_PROJECT') {
    sendResponse(message.reuseProject ? { success: false, error: 'Không mở được dự án đã gắn. Hãy kiểm tra link; hệ thống không tạo dự án mới.' } : openProjectFromDashboard());
    return false;
  }
  ...
  (async () => {
    try {
      await ensureProjectReady(Boolean(message.options?.reuseProject));
