...
import { workerStatus } from '@/lib/telegram/worker';
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ enabled: config.enabled, ready: isTelegramConfigured(), hasToken: !!config.token, hasSecret: config.secret.length >= 32, allowedUsers: config.allowedUsers, appUserId: config.userId || currentUser?.id || '', commands, worker: workerStatus() });
