    ...
    <>
      <header className="h-16 glass-panel border-b border-slate-800/80 sticky top-0 z-30 px-3 sm:px-6 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {/* Clickable Extension Connection Badge */}
          <button
            onClick={() => setShowInstallModal(true)}
            title="Bấm để xem hướng dẫn & tải Extension Chrome"
          ...
          >
            ...
            <span className="hidden lg:inline">Extension {isExtConnected ? '● Đã kết nối' : '📥 Tải Extension'}</span>
            <span className="lg:hidden">{isExtConnected ? 'Ext On' : 'Cài Ext'}</span>
          </button>
